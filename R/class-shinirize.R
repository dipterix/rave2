shinirize <- function(input, output, session, container, app_data, adapter){
  ...
  lapply(container$dynamic_script, function(s){
    tryCatch({
      ...
      rave_error("Error in {deparse1(e$call)}: {e$message}")
      print(e)
      traceback(e)
    })
    invisible()
  ...
  initialize_module <- function(){
    ...
    for(comp in container$input_components){
      tryCatch({
        ...
        rave_error("Error while updating input {comp$input_id} - {deparse1(e$call)}: {e$message}")
  ...
  execute_main <- function(){
    ...
    tryCatch({
      ...
      rave_error("Module run into error: {deparse1(e$call)}: {e$message}")
      FALSE
