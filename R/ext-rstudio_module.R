...
create_template <- function(path, ...){
  ...
  # ensure path exists
  dir_create2(path)
  dir_create2(file.path(path, 'inst', 'tools'))
  dir_create2(file.path(path, 'inst', 'modules', MODULEID))
  dir_create2(file.path(path, 'R'))
  ...
  # migrate template
  template_dir = system.file('rstudio/templates/project', package = pkg_name)
