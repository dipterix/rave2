...
Shiny.addCustomMessageHandler('rave_add_class', (data) => {
  ...
  if(!$(selector).hasClass(class_str)){
    $(selector).addClass(class_str);
  }
});


Shiny.addCustomMessageHandler('rave_remove_class', (data) => {
  const selector = data.selector,
        class_str = data['class'];
  ...
  if($(selector).hasClass(class_str)){
    $(selector).removeClass(class_str);
  }
});
